  ...
  try {
    ...
    const hostname =
      typeof location !== 'undefined' && location.hostname
        ? location.hostname
        : '127.0.0.1';
    let wsUrl = 'ws://127.0.0.1:' + BUILD.CONTROL_PORT + BUILD.WS_PATH;
    if (typeof location !== 'undefined' && location.host) {
      if (!location.port || location.port === '80' || location.port === '443' || location.port === '8080') {
        wsUrl = wsProto + '//' + location.host + BUILD.WS_PATH;
      } else {
        wsUrl = wsProto + '//' + hostname + ':' + BUILD.CONTROL_PORT + BUILD.WS_PATH;
      }
    }
